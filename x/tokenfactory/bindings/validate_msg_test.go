...
import (
	...
	wasmbinding "github.com/jmesworld/core/v17/x/tokenfactory/bindings"
...
func TestCreateDenom(t *testing.T) {
	...
	fundAccount(t, ctx, jmesapp, actor, actorAmount)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			_, gotErr := wasmbinding.PerformCreateDenom(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, actor, spec.createDenom)
			// then
			if spec.expErr {
...
func TestChangeAdmin(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// Setup
			jmesapp, ctx := SetupCustomApp(t, tokenCreator)

			// Fund actor with 100 base denom creation fees
			actorAmount := sdk.NewCoins(sdk.NewCoin(types.DefaultParams().DenomCreationFee[0].Denom, types.DefaultParams().DenomCreationFee[0].Amount.MulRaw(100)))
			fundAccount(t, ctx, jmesapp, tokenCreator, actorAmount)

			_, err := wasmbinding.PerformCreateDenom(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, tokenCreator, &bindings.CreateDenom{
				Subdenom: validDenom,
			})
			require.NoError(t, err)

			err = wasmbinding.ChangeAdmin(&jmesapp.AppKeepers.TokenFactoryKeeper, ctx, spec.actor, spec.changeAdmin)
...
func TestMint(t *testing.T) {
	...
	fundAccount(t, ctx, jmesapp, creator, tokenCreationFeeAmt)

	// Create denoms for valid mint tests
	validDenom := bindings.CreateDenom{
		Subdenom: "MOON",
	}
	_, err := wasmbinding.PerformCreateDenom(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, creator, &validDenom)
	require.NoError(t, err)

	emptyDenom := bindings.CreateDenom{
		Subdenom: "",
	}
	_, err = wasmbinding.PerformCreateDenom(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, creator, &emptyDenom)
	require.NoError(t, err)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformMint(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, creator, spec.mint)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestBurn(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			err := wasmbinding.PerformMint(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, creator, mintBinding)
			require.NoError(t, err)
			...
			err = wasmbinding.PerformMint(&jmesapp.AppKeepers.TokenFactoryKeeper, &jmesapp.AppKeepers.BankKeeper, ctx, creator, emptyDenomMintBinding)
			require.NoError(t, err)

			// when
			gotErr := wasmbinding.PerformBurn(&jmesapp.AppKeepers.TokenFactoryKeeper, ctx, creator, spec.burn)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
