#!/bin/bash
# Run this script to quickly install, setup, and run the current version of jmes without docker.
#
# Example:
...
export MONIKER="localjmes"
export KEYALGO="secp256k1"
export KEYRING=${KEYRING:-"test"}
export HOME_DIR=$(eval echo "${HOME_DIR:-"~/.jmes"}")
export BINARY=${BINARY:-jmesd}
...
from_scratch () {
  ...
  # jmes1efd63aw40lxf3n4mhf7dzhjkr453axurv2zdzk
  ...
  # jmes1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl
  ...
  BINARY init $MONIKER --chain-id $CHAIN_ID --default-denom ujmes
  ...
  # Gov
  update_test_genesis '.app_state["gov"]["params"]["min_deposit"]=[{"denom": "ujmes","amount": "1000000"}]'
  update_test_genesis '.app_state["gov"]["voting_params"]["voting_period"]="15s"'
  # staking
  update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="ujmes"'
  update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.050000000000000000"'  
  # mint
  update_test_genesis '.app_state["mint"]["params"]["mint_denom"]="ujmes"'
  # crisis
  update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom": "ujmes","amount": "1000"}'

  # Custom Modules
  # GlobalFee
  update_test_genesis '.app_state["globalfee"]["params"]["minimum_gas_prices"]=[{"amount":"0.002500000000000000","denom":"ujmes"}]'
  # Drip
  update_test_genesis '.app_state["drip"]["params"]["allowed_addresses"]=["jmes1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl","jmes1efd63aw40lxf3n4mhf7dzhjkr453axurv2zdzk"]'
  
  # TokenFactory
  # update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_fee"]=[{"denom":"ujmes","amount":"100"}]'
  update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_fee"]=[]'
  update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_gas_consume"]=2000000'

  # FeeShare
  update_test_genesis '.app_state["feeshare"]["params"]["allowed_denoms"]=["ujmes"]'
  ...
  # Builder keeper genesis state	
	...
	update_test_genesis '.app_state["builder"]["params"]["min_bid_increment"]["denom"]="ujmes"'
	update_test_genesis '.app_state["builder"]["params"]["min_bid_increment"]["amount"]="1000000"'
	update_test_genesis '.app_state["builder"]["params"]["reserve_fee"]["denom"]="ujmes"'
	update_test_genesis '.app_state["builder"]["params"]["reserve_fee"]["amount"]="1000000"'

  # Allocate genesis accounts
  BINARY genesis add-genesis-account $KEY 10000000ujmes,1000utest --keyring-backend $KEYRING
  BINARY genesis add-genesis-account $KEY2 1000000ujmes,1000utest --keyring-backend $KEYRING
  BINARY genesis add-genesis-account jmes1see0htr47uapjvcvh0hu6385rp8lw3emu85lh5 100000000000ujmes --keyring-backend $KEYRING

  BINARY genesis gentx $KEY 1000000ujmes --keyring-backend $KEYRING --chain-id $CHAIN_ID

  # Collect genesis tx
  BINARY genesis collect-gentxs

  # Run this to ensure jmesrything worked and that the genesis file is setup correctly
...
BINARY start --pruning=nothing  --minimum-gas-prices=0ujmes --rpc.laddr="tcp://0.0.0.0:$RPC"