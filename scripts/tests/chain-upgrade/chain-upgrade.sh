...
export OLD_BINARY=$ROOT/_build/jmesd_old
export NEW_BINARY=$ROOT/_build/jmesd_new
...
then
    ...
    make build
    cp /tmp/jmes/core/build/jmesd $ROOT/_build/jmesd_old
    cd $ROOT
fi
...
then
  make build
  cp build/jmesd $ROOT/_build/jmesd_new
fi
...
$OLD_BINARY add-genesis-account $($OLD_BINARY --home $CHAIN_HOME keys show val1 --keyring-backend test -a) 100000000000ujmes  --home $CHAIN_HOME
$OLD_BINARY gentx val1 1000000000ujmes --home $CHAIN_HOME --chain-id $CHAIN_ID --keyring-backend test
...
echo '{
  ...
  "summary": "Source Code Version https://github.com/jmesworld/core"
